...
//$sql = mysql_query("SELECT DATE_FORMAT( `time` , '%d.%m' ) AS time, ROUND( AVG( value ) , 0 ) AS value FROM `temp` WHERE name_id =1 GROUP BY DATE_FORMAT( `time` , '%d.%m' )");
...
//}
 ...
 $MyData = new pData();  
 for($i=0;$i<=5;$i++) { $MyData->addPoints(rand(1,15),"Probe 1"); }
 ...
 /* Draw the scale */
 $scaleSettings = array("XMargin"=>1,"YMargin"=>0,"Floating"=>TRUE,"GridR"=>200,"GridG"=>200,"GridB"=>200,"GridAlpha"=>100,"DrawSubTicks"=>TRUE,"CycleBackground"=>TRUE);
